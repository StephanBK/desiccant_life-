...
const srcText = (contrib) => {
  const c = contrib.life, per = contrib.life_hours > 8760 * 1.05 ? contrib.life_hours / 8760 : 1
  if (c.room_pct !== null && c.outdoor_pct !== null) return `water from room ${Math.round(c.room_pct)} % · outdoor ${Math.round(c.outdoor_pct)} %`
  const g = (x) => `${x < 0 ? '−' : '+'}${Math.round(Math.abs(x) / per)} g${per > 1 ? '/yr' : ''}`
  return `water: room ${g(c.room_g)} · outdoor ${g(c.outdoor_g)}`
}
        ...
        <main className="content">
          {!railOpen && (
            <div className="summary">
              ...
              {result?.contributions && <span className="srcsum">{srcText(result.contributions)}</span>}
